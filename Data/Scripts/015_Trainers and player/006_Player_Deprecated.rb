...
class PokeBattle_Trainer
  ...
  def self.convert(trainer)
	...
	Console.echoln trainer.name.to_s + "," + trainer.trainertype.to_s
    ...
    trainer.seen.each_with_index { |value, i| ret.pokedex.set_seen(PokeBattle_Pokemon.convertSpecies(i), false) if value }
    trainer.owned.each_with_index { |value, i| ret.pokedex.set_owned(PokeBattle_Pokemon.convertSpecies(i), false) if value }
    trainer.formseen.each_with_index do |value, i|
      species_id = GameData::Species.try_get(PokeBattle_Pokemon.convertSpecies(i))&.species
    ...
    if trainer.shadowcaught
      trainer.shadowcaught.each_with_index do |value, i|
        ret.pokedex.set_shadow_pokemon_owned(PokeBattle_Pokemon.convertSpecies(i)) if value
      end
    end
