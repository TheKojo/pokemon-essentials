module GameData
  class Species
	...
	def self.check_graphic_file_kojo(path, species, form = 0, gender = 0, shiny = false, shadow = false, subfolder = "")
      try_subfolder = sprintf("%s/", subfolder)
      try_species = species
      try_form    = (form > 0) ? sprintf("_%d", form) : ""
	...
	def self.icon_filename_kojo(species, form = 0, gender = 0, shiny = false, shadow = false, egg = false)
      return self.egg_icon_filename(species, form) if egg
      return self.check_graphic_file_kojo("Graphics/Characters/", species, form, gender, shiny, shadow)
    end
